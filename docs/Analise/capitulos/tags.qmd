...
```{r, df-newtags}
df_tags_not_equal$tags_all <- df_diff$all
...
df_all_games_ <- dplyr::inner_join(df_all_games, df_all_games_newtags, by="app_id") |> 
  dplyr::select(-tags, -tags_extra)

data.table::fwrite(df_all_games_, here::here("data-raw/steam-data/db-created/db-all-games-tags-all.csv"))
