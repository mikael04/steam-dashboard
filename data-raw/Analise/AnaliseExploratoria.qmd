...
games_fulljoin <- dplyr::full_join(df_games, df_selected_gen, by = "app_id") |> 
  dplyr::select(app_id, name_games = name.x, rating, positive_ratio, user_reviews, name_sel = name.y, overall, positive, negative, recommendations)
...
df_bind <- rbind(df_games, df_antijoin_games)
...
```{r, addingInfo}
df_info <- jsonlite::fromJSON("data-raw/steam-data/db-3/games_metadata.json")
library(jsonlite)
out <- lapply(readLines("data-raw/steam-data/db-3/games_metadata.json"), fromJSON) |> as.data.frame()
df_out <- data.frame(unlist(out))
df_bind <- df_

paste0(out[[1]][[3]], collapse = ",")
