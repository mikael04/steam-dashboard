...
Agora que já uni as duas bases, e tenho uma quantidade mais razoável de jogos a serem analisados, precisarei definir qual será a forma de avaliar os estilos/gêneros/categorias dos jogos, por ser uma coluna presente em ambas as bases, a variável `tags` e é uma ótima candidata, porém ela apresenta um número de possibilidades bastante grande e distinta, o que poderia ser prejudicial à análise. Já as outras variáveis `categories` e `genres` que são mais específicas, não estão presentes em aproximadamente 20% da base de dados. A análise será feita inialmente nas três para posterior decisão de qual ou quais usar.
...
## Separamos os que possuem apenas tamanhos diferentes de tags, a ideia inicial é analisar o que temos de diferente, e se for apenas uma ou outra tag a mais, utilizar a combinação destas tags
df_tags_not_equal_check <- tags_dif_ids[check_lenght == F]

## Os que tem mesmo tamanho mas não tem mesmas tags, provavelmente são elementos em ordens diferentes, então checarei isso
...
```{r, tags-diff-size}
# df_tags_not_equal_check <- df_tags_not_equal_check |> 
#   dplyr::mutate(tags_extra = gsub("-", " ", tags_extra))
library(purrr)
...
for(i in 1:length(vector_tags)){
  ...
  pluck(diff_elem, i, 3) <- pluck(df_tags_not_equal_check, 1, i)
}
...
pattern_first <- c("1 - ", "Roguelike", "Roguelite", "Base Building", "Football (Soccer)", "Football (American)")
...
# df_diff[] <- lapply(df_diff, function(x) { attributes(x) <- NULL; x})
...
df_diff$tags <- as.character(tags)
...
order_strings <- function(strings){
  # teste_interno <- F
  # if(teste_interno){
#   strings <- "Anime,Dating Sim,Female Protagonist,Otome,Romance,Simulation,Singleplayer,Story Rich,Visual Novel,Choices Matter"
  # }
  all_ordered <- strings |> 
    purrr::map(\(x) stringr::str_sort(stringr::str_split_1(x, pattern = ",")))
...
df_diff <- df_diff |> 
  ...
  dplyr::rowwise() |> 
  dplyr::mutate(all = order_strings(all)) |> 
  dplyr::ungroup()
